...
tellraw @a[tag=debug,scores={debugLogPriority=201..}] {"text":"","extra": [{"text":"[mGrid] ","color":"light_purple","hoverEvent": {"action":"show_text","contents": "minihilly Grid\nDebug Info"}},{"text":"minihilly_2d_grid:destroy/grid_without_uuid"}]}

# grid manager
execute as @e[tag=minihilly_grid.grid_manager] if score @s minihilly_grid.id = $grid_to_destroy minihilly_grid.config run function minihilly_2d_grid:destroy/teardown